...
export default function AdminLogin() {
  ...
  const handleGoogleSignIn = async () => {
    ...
    try {
      ...
      if (result?.error) {
        setError('Access denied. You must be a member of the duanesburgboe group to access the admin panel.')
      } else if (result?.ok) {
        router.push('/admin')
      }
    } catch (error) {
        ...
        <div className="bg-white rounded-lg shadow-md p-8">
          <div className="text-center">
            ...
            <p className="text-gray-600 mb-8">
              Sign in with your Duanesburg Google account to access the Hall of Fame admin panel.
            </p>
          </div>
          ...
          <div className="mt-6 text-center">
            <p className="text-xs text-gray-500">
              Only members of the duanesburgboe group can access this admin panel.
            </p>
          </div>
        </div>
      </div>
